...
public class DetailImageActivity extends AppCompatActivity
{
    ...
    protected void onCreate(Bundle savedInstanceState) 
    {
       ...
       // DragPhotoView dragPhotoView = (DragPhotoView) findViewById(R.id.dragPhotoView);
        viewPager = (ViewPager) findViewById(R.id.dragViewPage);
        
        String url = getIntent().getStringExtra("哈哈");
       // Glide.with(this).load(url).into(dragPhotoView);
        imgUrls = getIntent().getStringArrayListExtra("imgList");
        position = getIntent().getIntExtra("position", 0);
        //获取数据
        mPhotoViews = new DragPhotoView[imgUrls.size()];
        for (int i = 0; i < mPhotoViews.length; i++) 
        {
            mPhotoViews[i] = (DragPhotoView) View.inflate(this, R.layout.item_view_pager, null);
            //加载图片
            String imgUrl = imgUrls.get(i);
            if (isGif(imgUrl))
            {

                Glide.with(this).load(imgUrl).asGif().diskCacheStrategy(DiskCacheStrategy.SOURCE).into(mPhotoViews[i]);
                Log.d("url为：", imgUrl);
            }
            else 
            Glide.with(this).load(imgUrls.get(i)).into(mPhotoViews[i]);
